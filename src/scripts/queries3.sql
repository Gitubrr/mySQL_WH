...
FROM Order1 
WHERE Is_Done = 1 
  AND Employee_ID IN (SELECT Employee_ID FROM Employee WHERE Spec = 'student');
SELECT per.Last_Name
FROM Person per
JOIN Employee e ON per.Person_ID = e.Person_ID
WHERE e.Employee_ID NOT IN (SELECT DISTINCT Employee_ID FROM Order1);
SELECT 
    s.Name AS 'Service', 
...
SELECT Description AS 'All_Comments' FROM Owner WHERE Description IS NOT NULL
UNION
SELECT Comments FROM Order1 WHERE Comments IS NOT NULL
UNION
SELECT Description FROM Pet WHERE Description IS NOT NULL;
SELECT per.First_Name, per.Last_Name
FROM Person per
JOIN Employee e ON per.Person_ID = e.Person_ID
WHERE EXISTS (
    SELECT 1 FROM Order1 o 
    WHERE o.Employee_ID = e.Employee_ID AND o.Mark = 4
);
